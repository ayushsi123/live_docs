...
export const updateDocument = async (roomId: string, title: string) => {
    try {
        ...
        revalidatePath(`/documemts/${roomId}`)
...
export const removeCollaborator = async ({ roomId, email }: {roomId:string,email: string}) =>{
    try {  
        ...
        console.log(`Error happened while removing colloborator from document access: ${error}`)
        }
}
...
export const deleteDocument = async(roomId:string)=>{
    try {
        ...
        console.log(`Error happened while deleting room from liveblocks: ${error}`)
    }
}